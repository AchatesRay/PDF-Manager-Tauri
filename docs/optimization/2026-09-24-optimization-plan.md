...
| 维度 | 终态 |
|------|------|
| 分支 | `main` = `df9a5ae` = `origin/main`（0/0 同步，工作树干净） |
| 进度 | Phase 0–5 全部完成并 push（4 个 commit：`4d3360e` fix / `112c22d` docs / `ebec4ce` chore / `df9a5ae` docs） |
...
1. **Q-1 搜索质量**：jieba 整词精确匹配致中文子串查询漏检（`北斗` 查不到「北斗七星」）——中文搜索是产品卖点，建议优先
2. **Q-2 搜索质量**：OCR 截断长 ASCII 词（`NewTokenBeta` → `NewTokenBe`）导致关键词漏检（模型侧弱点，评估成本后再定）
3. **拆大组件**：FolderTree ~684 行 / PdfList ~600 行
4. **队列持久化**：重启恢复 pending 任务
5. **OCR 单测套件**（2026-03-26 计划 Chunk 7 残余）
6. **可配置预处理**（2026-03-26 计划 Chunk 6 残余）
7. **搜索测试 flake 排查**：`test_index_and_search` 曾 1 次偶发失败
